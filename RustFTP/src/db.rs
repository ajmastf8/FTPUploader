...
pub fn init_database(db_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    println!("🗄️  Initializing SQLite database at: {}", db_path.display());
    ...
    // PRAGMA statements return results, so we need to consume them
...
pub fn save_hash(
    ...
    println!("💾 DB: Saved hash for {}/{} (hash: {})", remote_dir, filename, hash);
    Ok(())
}
...
pub fn load_hashes_for_config(config_id: &str) -> Result<HashMap<String, u64>, Box<dyn std::error::Error>> {
    ...
    println!("📊 DB: Loaded {} hashes for config {}", hashes.len(), config_id);
    Ok(hashes)
}
...
pub fn cleanup_stale_files(
    ...
    if deleted > 0 {
        println!("🧹 DB: Cleaned up {} stale file entries for config {}", deleted, config_id);
    }
...
pub fn migrate_from_hash_file(
    ...
    if !hash_file_path.exists() {
        println!("ℹ️  No legacy hash file found at: {}", hash_file_path.display());
        return Ok(0);
    }

    println!("🔄 Migrating legacy hash file to database: {}", hash_file_path.display());
    ...
    for (line_num, line) in content.lines().enumerate() {
        ...
        let (remote_dir, filename, file_size, mod_time, hash) = if parts.len() >= 5 {
            ...
            println!("⚠️  Line {}: Invalid format, skipping: {}", line_num + 1, line);
            continue;
        };
    ...
    println!("✅ Migrated {} entries from legacy hash file", migrated);
    ...
    if let Err(e) = std::fs::rename(hash_file_path, &backup_path) {
        println!("⚠️  Could not rename legacy hash file: {}", e);
    } else {
        println!("📦 Legacy hash file backed up to: {}", backup_path.display());
    }
...
pub fn delete_config_data(config_id: &str) -> Result<usize, Box<dyn std::error::Error>> {
    ...
    println!("🗑️  DB: Deleted {} entries for config {}", deleted, config_id);
    Ok(deleted)
}
